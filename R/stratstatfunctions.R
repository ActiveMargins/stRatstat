...
launchstRatstat <- function(){
  ...
  server <- function(input, output) {
    ...
    values <- reactiveValues(df=tibble(x=-5,y=-5, pt=1)) #points to represent the grain size profile values x=position on x-axis of image, y=position on y-axis of image, pt=point number for the logged points
    topbottom <- reactiveValues(df=tibble(x=-5,y=-5,PtLabel="Example")) #points picked to locate the top and bottom of the section. x=position on the x-axis of the image, y=position on the y-axis of the image, label=string variable to lable as top and bottom
    gsmarkers <- reactiveValues(df=tibble(x=-5,y=-5,pt=1)) #points to represent the grain size division at the base of the section, x=position on the x-axis of the image, y=position on the y-axis of the image, pt=pt number for the logged points
    bedtops_raw <- reactiveValues(df=tibble(x=-5,y=-5, pt=1, BedTop=1)) #points picked to represent bed boundaries, x=position on the x-axis of the image, y=position on the y-axis of the image, pt=the number of the picked points, bedtop=numeric value of 1, which is used later to signify a the top of a bed in a discretized layer
    ...
    sedstrat_raw <- reactiveValues(df=tibble(ymin_raw=-5,ymax_raw=-2, SedStruct="Example")) #recorded brushes of logged sedimentary structure intervals, y-min_raw and y-max_raw=the y-axis locations of brushes on the ggplot, SedStruct=the selected sedimnentary structure category from the list
    faciesnames <- reactiveValues(df=tibble(name = "Example Facies")) #a reactive list of names for sedimentary stucture categories
    facies_raw <- reactiveValues(df=tibble(ymin_raw=-5,ymax_raw=-2, FacName="Example", FacBlockNum=0)) #recorded brushes of logged facies intervals, y-min_raw and y-max_raw=the y-axis locations of brushes on the ggplot, FacName=the selected facies category from the list, FaciesBlockNum=the incrementing number of different instances of logged facies - used to summarize data
    elementnames <- reactiveValues(df=tibble(name = "Example Element")) #a reactive list of names for sedimentary stucture categories
    element_raw <- reactiveValues(df=tibble(ymin_raw=-5,ymax_raw=-2, ElementName="Example", ElementBlockNum=0)) #recorded brushes of logged element intervals, y-min_raw and y-max_raw=the y-axis locations of brushes on the ggplot, ElementName=the selected element category from the list, ElementBlockNum=the incrementing number of different instances of logged elements - used to summarize data
    ...
    elementset_raw <- reactiveValues(df=tibble(ymin_raw=-5,ymax_raw=-2, ElementSetName="Example", ElementSetBlockNum=0)) #recorded brushes of logged element set intervals, y-min_raw and y-max_raw=the y-axis locations of brushes on the ggplot, ElementSetName=the selected element set category from the list, ElementSetBlockNum=the incrementing number of different instances of logged element sets - used to summarize data
    ...
    output$plot2 <- renderPlot({ #left hand plot for manipulating the view on the right hand plot
      if(is.null(input$files)){} # if there's no file input don't plot anything
        ...
        ggplot(null$df,aes(x=nullx,y=nully)) + #null data required to view the ggplot
          annotation_raster(LoadedImage(),ymin=2,ymax = height(LoadedImage()), xmin=0, xmax=width(LoadedImage())) + #loaded image in the background
          geom_point() +
    ...
    observeEvent(input$delete_row, { #delete the selected row(s) from the dataframe selected by the radio buttons
